...
def bfs(visited, maps, rows, cols):
    ...
    while queue:
        cx, cy, is_break = queue.popleft()
        if cx == rows-1 and cy == cols-1:
            return visited[cx][cy][is_break]
        else:
            for mv in mv_list:
                nx = cx + mv[0]
                ny = cy + mv[1]
                if 0 <= nx < rows and 0 <= ny < cols:
                    ...
                    if maps[nx][ny] == '0' and visited[nx][ny][is_break] == 0:
                        visited[nx][ny][is_break] = visited[cx][cy][is_break] + 1
                        queue.append([nx, ny, is_break])
                    # 벽을 부순 적이 없는데 벽을 보았으면 거기로 갔을 리가 없다.
                    elif maps[nx][ny] == '1' and is_break == 0:
