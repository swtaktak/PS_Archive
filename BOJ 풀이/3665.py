...
for _ in range(T):
    ...
    visited = [False for _ in range(team + 1)]
    ...
    # case 1. 애초부터 전체 사이클이어서, 랭크 계산 불가
    if len(q) == 0:
        no_cycle_flag = False
    # case 2. 전체 cycle은 아닌 경우.
    else:
        while q:
            ...
            for nt in next_team_list:
                ...
                if indegree[nt] == 0:
                    q.append(nt)
    # cycle 발생하거나, 이건 통과 안했는데 중간에 도는 형태일경우....
    if not no_cycle_flag or len(result_list) != team:
        print("IMPOSSIBLE")
